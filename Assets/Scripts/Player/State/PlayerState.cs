    ...
    protected void Rotate(float x, float z)
    {
        if (_playerInput.camera != null)
        {
            ...
            if (moveDirection != Vector3.zero)
            {
                ...
                var originX = _playerController.transform.rotation;
                _playerController.transform.rotation = Quaternion.Slerp(originX, Quaternion.LookRotation(moveDirection), 0.1f);
            }
        }
    }
